...
const TermsServiceModal = ({ isOpen, onClose, onAgree } : any) => {
  const [form, setForm] = useState({ name: "", signature: "", email : "", date: Date.now().toString() });

  const isFormComplete = form.name && form.signature && form.date;

  return (
    <ModalBase isOpen={isOpen} onClose={onClose}>
      <div className="flex justify-between w-full bg-inherit items-center mt-[1rem] px-8 xl:py-2">
      ...
      <div className="overflow-y-auto max-h-[505px] xl:max-h-[500px] 2xl:max-h-[620px] w-full px-8 py-4 xl:space-y-2 2xl:space-y-3">
        ...
        <div className="flex flex-col md:grid-cols-2 md:grid font-[300] w-full gap-[1.5rem]">
          <input
            type="text"
